...
function showSlides() {
  ...
  var dots = document.querySelector(".pointer");
...
function selecionarSlide(n) {
  ...
  // Faço o loop for e verifico se o display: none; está no css do slide //
  for (i = 0; i < slides.length; i++) {
      slides[i].style.display = "none";  
  }
  
  //Faço o loop for e já mudo os resultados para active nos ponteiros para eles ficarem selecionados//
  for (i = 0; i < dots.length; i++) {
      dots[i].className = dots[i].className.replace(" active", "");
  }
